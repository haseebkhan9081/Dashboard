...
const SelectQuotationWorkSheet = () => {
  ...
  React.useEffect(() => {
    ...
    const sheetValue = url.searchParams.get('QuotationWorkSheet');
  ...
  const handleSelectChange = (selectedOption: File | null) => {
    ...
    const url = new URL(window.location.href);
    if (selectedOption) {
      url.searchParams.set('QuotationWorkSheet', String(selectedOption.value));
    } else {
      url.searchParams.delete('QuotationWorkSheet');
    }
  ...
  return (
    <div className="w-full justify-center items-center flex space-y-4 flex-col">
      <h3 className="text-slate-500">Quotation WorkSheet:</h3>
      <Select
        value={selectedValue}
        ...
        placeholder="Select Quotation WorkSheet..."
        className="w-[280px]"
      />
    </div>
  );
...
export default SelectQuotationWorkSheet;
