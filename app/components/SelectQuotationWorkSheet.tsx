...
const fetchAllFiles = async (sheetId: string): Promise<File[]> => {
  const response = await fetch(`http://localhost:3002/api/sheet/worksheets?sheetId=${sheetId}`);
  if (!response.ok) {
...
const SelectQuotationWorkSheet = () => {
  ...
  const handleSelectChange = (value: string) => {
    ...
    const url = new URL(window.location.href);
    url.searchParams.set('QuotationWorkSheet', value);
  ...
  return (
    <Select value={String(selectedValue)} onValueChange={handleSelectChange}>
      <SelectTrigger className="w-[280px]">
        <SelectValue placeholder="Select Quotation WorkSheet..." />
      </SelectTrigger>  
      <SelectContent>
        <SelectGroup>
          {data?.map((file, index) => (
